...
<div class="body d-flex py-lg-3 py-md-2">
    <div class="container-xxl">
        <div class="row align-items-center mb-4">
            <div class="col-md-6"><h3 class="fw-bold">Leaders</h3></div>
            <div class="col-md-6 text-end">
                <a href="{{ route('leaders.create') }}" class="btn btn-primary">
                    <i class="fa fa-plus"></i> Add Leader
                </a>
            </div>
        </div>
                ...
                <table class="table table-bordered align-middle" id="myDataTable">
                    <thead>
                        <tr>
                            ...
                            <th data-priority="2">Designation</th>
                    ...
                    <tbody>
                        @forelse($leaders as $leader)
                            <tr data-name="{{ $leader->leader_name }}">
                                <td>{{ $loop->iteration }}</td>
                                <td>
                                    @if($leader->leader_image)
                                        <img src="{{ $leader->leader_image_url }}" alt="{{ $leader->leader_name }}"
                                             style="width:60px;height:60px;object-fit:cover;border-radius:4px;">
                                    @else
                                        <span class="text-muted">&mdash;</span>
                                    @endif
                                </td>
                                <td>{{ $leader->leader_title }}</td>
                                <td>{{ $leader->leader_name }}</td>
                                <td>{{ $leader->leader_designation }}</td>
                                <td>
                                    @if($leader->trashed())
                                        ...
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="form-check form-switch mb-0">
                                                <input class="form-check-input toggle-status" type="checkbox" role="switch"
                                                       ...
                                                       data-url="{{ route('leaders.toggle_status', $leader->id) }}"
                                                       {{ $leader->is_active ? 'checked' : '' }}>
                                            </div>
                                            <span class="status-label small fw-semibold {{ $leader->is_active ? 'text-success' : 'text-secondary' }}">
                                ...
                                <td class="text-nowrap">
                                    @if($leader->trashed())
                                        <form action="{{ route('leaders.restore', $leader->id) }}" method="POST" class="d-inline-block">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    @else
                                        <a href="{{ route('leaders.edit', $leader->id) }}" class="btn btn-sm btn-outline-primary" title="Edit">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>
                                        <button type="button" class="btn btn-sm btn-outline-danger js-delete-btn"
                                                data-name="{{ $leader->leader_name }}"
                                                data-url="{{ route('leaders.delete', $leader->id) }}"
                                                title="Delete">
                        ...
                        @empty
                            <tr><td colspan="8" class="text-center text-muted py-4">No leaders yet. Click "Add Leader" to create one.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
...
<script>
    $(function() {
        ...
        $(document).on('change', '.toggle-status', function() {
            ...
            const leaderName = $chk.closest('tr').data('name') || 'Leader';
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.is_active) {
                        ...
                        showAppToast('success', `"${leaderName}" marked as Active.`);
                    } else {
                        $label.text('Inactive').removeClass('text-success').addClass('text-secondary');
                        showAppToast('info', `"${leaderName}" marked as Inactive.`);
                    }
                },
                error: function() {
